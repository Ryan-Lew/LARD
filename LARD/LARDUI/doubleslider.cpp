...
//    painter->drawText(QRectF(width() - maxtextWidth -2, 4, maxtextWidth,textHeight), maxValueString);
    ...
    //----- paint groove
    paintColoredRect(QRect(4,6,width() - 8,2),Qt::gray,painter);
    paintColoredRect(QRect(minPos + 4,6,maxPos - minPos,2),QColor(135,174,48),painter);

    //----- handle

    minHandleRegion = QRect(minPos ,0,8,14);
    maxHandleRegion = QRect(maxPos ,0,8,14);
